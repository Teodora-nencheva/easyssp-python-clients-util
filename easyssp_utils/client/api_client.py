...
class ApiClient:
    ...
    :param header_value: a header value to pass when making calls to
        the API.
    :param user_agent: the user agent name
    :param cookie: a cookie to include in the header when making calls
        to the API
    """

    PRIMITIVE_TYPES = (float, bool, bytes, str, int)
    NATIVE_TYPES_MAPPING = {  # noqa: RUF012
        "int": int,
        "long": int,
        "float": float,
        "str": str,
    ...
    def response_deserialize(
        ...
        try:
            if response_type == "bytearray":
                ...
                return_data = self.deserialize(response_text, response_type, content_type, custom_models)
        finally:
    ...
    def deserialize(self, response_text: str, response_type: str, content_type: str | None,
        ...
        return self.__deserialize(data, response_type, custom_models)

    def __deserialize(self, data, klass, custom_models: Any = None):
        ...
        if isinstance(klass, str):
            if klass.startswith("List["):
                ...
                sub_kls = m.group(1)
                return [self.__deserialize(sub_data, sub_kls, custom_models)
            ...
            if klass.startswith("Dict["):
                ...
                sub_kls = m.group(2)
                return {k: self.__deserialize(v, sub_kls, custom_models)
            ...
            if klass in self.NATIVE_TYPES_MAPPING:
                ...
                try:
                    ...
                    klass = getattr(custom_models, klass)
    ...
    def parameters_to_tuples(params, collection_formats):
        ...
        for k, v in params.items() if isinstance(params, dict) else params:
            if k in collection_formats:
                ...
                # MANUAL MODIFICATION
                # in requests with form data request body
                # the content type of each form data body part needs to be explicitly specified
                # otherwise it is not correctly interpreted by the API
                elif collection_format == "application/json":
                    new_params.append((k, (k, v.encode("utf-8"), "application/json")))
                else:
